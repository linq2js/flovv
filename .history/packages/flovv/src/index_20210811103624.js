...
export function createStore({
  ...
  function flow(fn) {
    ...
    let f = flows.get(fn);
    if (!f) {
      f = createFlow(fn, getState, flow, commands);
      flows.set(fn, f);
    }
    return f;
  }
...
export function createFlow(fn, getState, getFlow, commands) {
  ...
  let error;
  let promise;
  const dependencyFlows = new WeakSet();
  ...
  commands = {
    ...
    ref(payload, task) {
      ...
      targetArray.forEach((target, index) => {
        ...
        if (typeof target === "string") {
          stateProps.push([index, target]);
        } else {
          flows.push([index, target]);
        }
      });
      if (!promises.length) {
        return task.handleSuccess(isMultiple ? values : values[0]);
      }
      Promise.all(promises).finally(() => {
        if (lastError) {
          return task.handleError(lastError);
        }
        return task.handleSuccess(values);
      });
    },
    get(payload, task) {
      const isMultiple = Array.isArray(payload);
      const values = [];
      fillStateValues(Object.entries(isMultiple ? payload : [payload]), values);
      return task.handleSuccess(isMultiple ? values : values[0]);
    },
  };
  ...
  function resolveFlowData(flows, values, onDone) {
    function resolve() {
      const loadingFlows = flows.filter((flow) => flow.status === "loading");
      if (!loadingFlows.length) {
        flows.forEach(([index, flow]) => (values[index] = flow.data));
        return onDone();
      }
    }
  }

  function fillStateValues(props, values) {
