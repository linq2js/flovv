...
export default function flovv({
  ...
  const commands = {
    set(payload) {
      let changed = false;
      const changeTokens = {};
      Object.entries(payload).forEach(([state, value]) => {
        if (typeof value === "function") {
          value = value(currentState[state]);
        }
      });
    },
    emit(payload) {},
...
function createCancellationToken(parent) {
  let error;
  let active = true;
...
function createFlow(fn, getState, commands) {
  let stale = true;
  let status = "pending";
  let token = createCancellationToken();
  let data;
  let error;
  const props = new Map();
  const emitter = createEmitter();
  ...
  function start(payload) {
    ...
    let isAsync = true;
    const ct = (token = createCancellationToken());
    props.clear();
    stale = false;
    error = undefined;
    status = "loading";
