...
export function createStore({
  ...
  function flow(fn) {
    if (typeof fn === "string") {
      ...
      let cachedFn = flowFnCache.get(cacheKey);
      if (!cachedFn) {
        ...
        flowFnCache.set(cacheKey, cachedFn);
      }
    }
...
export function createFlow(fn, getState, commands) {
  ...
  function start(payload) {
    ...
    const task = createTask(
      ...
      (result) => {
        isAsync = false;
        return handleChange("success", result);
      },
      (error) => {
        isAsync = false;
        return handleChange("fail", undefined, error);
      }
    );
    task.onCancel(() => {
