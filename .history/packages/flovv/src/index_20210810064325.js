...
export function processFlow(iterator, payload, commands, task) {
  ...
  if (typeof value.next === "function") {
    return processFlow(
      ...
      createTask(task, (result) =>
        processFlow(iterator, result, commands, task)
      )
    );
  }
  ...
  if (value.fork) {
    ...
    return processFlow(
      iterator,
      isMultipleFork ? childTasks : childTasks[0],
      commands,
      task
    );
  }
  ...
  if (value.all) {
    const results = Array.isArray(value.all) ? [] : {};
  ...
  return commands[key](
    value[key],
    commands,
    createTask(task, (result) => processFlow(iterator, result, commands, task))
  );
}

function processMultipleTask(mode, target, task, onSuccess) {
