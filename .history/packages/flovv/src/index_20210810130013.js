...
function processFork(fork, commands, task) {
  ...
  forks.forEach(([key, forkedIterator]) => {
    // cancel forked flow
    const childTask = createTask(undefined, undefined, task.handleError);
    processFlow(forkedIterator, undefined, commands, childTask);
    childTasks[key] = childTask;
  });
  return isMultipleFork ? childTasks : childTasks[0];
}

function processExpression(value, task, commands) {
  ...
  if (value.fork) {
    return next(processFork(value.fork, commands, task));
  }
  ...
  return commands[key](value[key], task.child(next), commands);
}
...
function processAsync(mode, target, task, commands) {
  ...
  function handleChange(key, result, error, childTask) {
    ...
    if (mode === "any" || count >= entries.length) {
      dispose();
      if (mode === "any") {
        childTasks.forEach((x) => x !== childTask && x.cancel());
      }
