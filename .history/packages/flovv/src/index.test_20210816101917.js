...
test("kyed flow", () => {
  ...
  store.start(updateValue, "v1");
  store.start(updateValue, "v2");

  const f3 = store.flow(getValue, ["v1"]);
  const f4 = store.flow(getValue, ["v2"]);

  expect(f3).not.toBe(f1);
  expect(f4).not.toBe(f2);
});
