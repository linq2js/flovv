...
export function createStore({
  ...
  const commands = {
    ...
    when(payload, task) {
      ...
      const removeDisposeListeners = events.map(([target, check]) => {
        let removeListener;
        if (typeof target === "string") {
          removeListener = emitter.on(target, (e) => {
        ...
        // when: flow => watch flow state
          ...
          const f = flow(target);
          removeListener = f.watch(() => {});
        }
        return task.onDispose(removeListener);
      });

      function cleanup() {
        removeDisposeListeners.forEach((removeDisposeListener) => {
          const removeEventListener = removeDisposeListener();
          typeof removeEventListener === "function" && removeEventListener();
        });
      }
    },
  };
