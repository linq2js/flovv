  ...
  commands = {
    ...
    ref(payload, task) {
      ...
      targetArray.forEach((target, index) => {
        ...
        if (typeof target === "string") {
          stateProps.push({ index, prop: target });
          return;
        }
        return state[target];
      });
    ...
    get(payload, task) {
      const state = getState();
      const isMultiple = Array.isArray(payload);
      const propArray = isMultiple ? payload : [payload];
      const values = propArray.map((prop) => state[prop]);
      return task.handleSuccess(isMultiple ? values : values[0]);
    },
  };

  function fillStateValues(props, results) {
    const state = getState();
    props.map(([index, prop]) => (results[index] = state[prop]));
  }
