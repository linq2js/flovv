...
export function processFlow(iterator, payload, commands, task) {
  ...
  return processCommand(value, task, commands);
}

function processCommand(value, task, commands) {
...
function waitFor(mode, target, task, commands) {
  const results = Array.isArray(target) ? [] : {};
  const entries = Object.entries(target);

  function handleChange(result, error) {}
}
